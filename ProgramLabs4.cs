...
namespace edu_simple
{
    ...
    {
        string GetId(); // просто вернуть строковый ID
    }

    public interface IPrintable
    {
        void Print(); // вывести себя на экран
    }

    // интерфейс для работы с оценками (добавить/очистить)
    ...
    // модели
    public class Student : IHasId, IPrintable, IMarkEditable // реализуем СРАЗУ несколько интерфейсов
    {
        public string Id;
        ...
        {
            // просто, безз проверок на существование предмета
            Marks[subject] = mark;
        }
    ...
    class Program
    {
        ...
        static void Main()
        {
            ...
            while (true)
            {
                ...
                switch (cmd)
                {
                    ...
                    case "10": TestStudentInterfacesWithDelegate(); break; // тест через многоадресный делегат
        ...
        static void Menu()
        {
            ...
            Console.WriteLine("10) Тест методов студента (интерфейсы + многоадресный делегат)"); // новое
        ...
        static void PutMark()
        {
            ...
            // теперь через интерфейсный метод (но он просто пишет в словарь)
            st.PutMark(subj, m);
        ...
        // Новый пункт меню: тест интерфейсов через многоадресный делегат
        static void TestStudentInterfacesWithDelegate()
        {
            ...
            // Собираем многоадресный делегат:
            StudentAction tests = ActionShowId;    // проверим IHasId
            tests += ActionPrint;                  // проверим IPrintable
            tests += ActionPutDemoMark;            // проверим IMarkEditable.PutMark
            tests += ActionPrint;
            tests += ActionClearMarks;             // проверим IMarkEditable.ClearMarks
            tests += ActionPrint;

            Console.WriteLine("=== Запуск тестов (делегат вызывает несколько методов подряд) ===");
            tests(st);
