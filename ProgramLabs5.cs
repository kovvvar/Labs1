...
{
    // Интерфейсы (как в исходном коде)
    ...
    // События для ошибок (простая модель)

    // Аргументы события про ошибку (что случилось и текст ошибки)
    ...
    public class ErrorNotifierBase
    {
        ...
        // Публичный помощник (чтобы породные классы и демонстрации могли вызывать единообразно)
    ...
    // и слегка меняет формат сообщений (добавляет префикс).
    public class ErrorNotifierDerived : ErrorNotifierBase
    {
        ...
        {
            // добавим простой префикс, чтобы было видно, что событие "переопределено"
            var handler = _backing;
            if (handler != null)
            {
                var msg = "[DerivedEvent] " + text;
                handler(this, new ErrorEventArgs(kind, msg));
            }
        }
    }

    // ===== Модели из исходного кода =====
    public class Student : IHasId, IPrintable, IMarkEditable
    {
        ...
        public string GetId() { return Id; } // IHasId

        public void Print()                  // IPrintable
        {
            Console.WriteLine(ToString());
        }

        public void PutMark(string subject, int mark) // IMarkEditable
        {
            // просто, без сложных проверок
            Marks[subject] = mark;
        }

        public void ClearMarks() // IMarkEditable
        {
            Marks.Clear();
        }
    }
    ...
    // Класс, который демонстрирует 7 исключений и сообщает об ошибках через события
    public class ExceptionDemo
    {
        ...
        // Каждый метод пробует вызвать исключение и перехватывает его, чтобы передать в событие
        ...
        public void Demo_ArrayTypeMismatch()
        {
            try
            {
                // создаём массив строк, но обращаемся как к object[]
                string[] sarr = new string[2];
                object[] oarr = sarr;      // ковариантность массивов
                oarr[0] = 123;             // попытка положить int в string[] -> ArrayTypeMismatchException
            }
        ...
        public void Demo_InvalidCast()
        {
            ...
            {
                object obj = "hello";
                // Пытаемся привести строку к Student -> InvalidCastException
        ...
        public void Demo_Overflow()
        {
            try
            {
                // Переполнение в checked-контексте даёт OverflowException
                checked
                {
        ...
        public void Demo_OutOfMemory()
        {
            try
            {
                // Реально вызывать нехватку памяти опасно (крах процесса), поэтому покажем как будто:
                throw new OutOfMemoryException("Симулированная нехватка памяти для демонстрации обработки.");
            }
        ...
        public void Demo_StackOverflow()
        {
            try
            {
                // Реальный StackOverflow нельзя безопасно перехватить, поэтому бросаем вручную для учебной демонстрации:

                throw new StackOverflowException("Симулированное переполнение стека для демонстрации обработки.");
            }
    ...
    // ===== Основная программа (добавлен пункт меню №11) =====
    class Program
    {
        static List<Institute> institutes = new List<Institute>();
        static int autoId = 1; // простая авто-нумерация студентов

        // Многоадресный делегат для проверки методов студента
        delegate void StudentAction(Student s);

        // Один общий объект-нотификатор (производный), чтобы видеть, что событие переопределено
        ...
        static void Main()
        {
            ...
            // подпишемся на событие об ошибке (через анонимный метод)
            notifier.ErrorHappened += (sender, e) =>
            {
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine($"[ОШИБКА] Тип: {e.Kind}");
            ...
            while (true)
            {
                ...
                switch (cmd)
                {
                    ...
                    case "11": DemoExceptions(); break; // НОВОЕ: демонстрация 7 исключений
        ...
        static void Menu()
        {
            ...
            Console.WriteLine("11) Демонстрация обработки ошибок (7 исключений)");
        ...
        // Тест интерфейсов через многоадресный делегат (как было)
        ...
        // Методы под делегат
        static void ActionShowId(Student s)
        {
            Console.WriteLine("ID студента: " + s.GetId());
        }

        static void ActionPrint(Student s)
        {
            s.Print();
        }
        ...
        {
            s.PutMark("Демо", 5);
            Console.WriteLine("Поставили оценку по предмету 'Демо': 5");
        }

        static void ActionClearMarks(Student s)
        {
            s.ClearMarks();
            Console.WriteLine("Очистили все оценки у студента.");
        }

        // Демонстрация всех 7 исключений
        static void DemoExceptions()
        {
            Console.WriteLine("=== Демонстрация обработки ошибок через событие (наследование) ===");
            var demo = new ExceptionDemo(notifier);

            demo.Demo_StackOverflow();      // 1) StackOverflowException (симуляция)
            ...
            demo.Demo_OutOfMemory();        // 6) OutOfMemoryException (симуляция)
